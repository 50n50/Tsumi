<script>
  ...
  $: if (currentStatic) {
    bannerImages.value = {
      ...
      rotated:
        !(currentStatic.bannerImage || currentStatic.trailer?.id) &&
        settings.value.adult === "hentai" &&
        settings.value.hentaiBanner,
      id: currentStatic.id,
    };
    // Check current scroll position instead of blindly resetting
    // This is handled by the scroll container in HomePage
  }
